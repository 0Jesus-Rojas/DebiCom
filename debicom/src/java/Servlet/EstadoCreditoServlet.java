...
public class EstadoCreditoServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        Integer usuario = AuthUtil.getIdUsuario(request);
        if (usuario == null) { response.sendRedirect(request.getContextPath() + "/index.jsp"); return; }
        try {
            Integer cliente = usuarioDAO.obtenerIdClientePorUsuario(usuario);
            if (cliente == null) { response.sendError(404, "El usuario no tiene perfil de cliente."); return; }
            request.setAttribute("solicitudes", solicitudDAO.listarPorCliente(cliente));
            request.getRequestDispatcher("/estado-credito.jsp").forward(request, response);
        } catch (RuntimeException e) {
            log("Error consultando estado de préstamos", e);
            response.sendError(500, "No fue posible consultar el estado de los préstamos.");
        }
    }
}
